...
class Gemma3Attention(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Match HF Gemma3: scaling = query_pre_attn_scalar ** -0.5
    ...
    def _make_sliding_window_mask(self, q_len: int, kv_len: int, dtype: jnp.dtype) -> jnp.ndarray:
        ...
        # Build causal mask using absolute query/key positions so decode with cache
        # (q_len=1, kv_len>>1) can still attend to all past keys.
        q_pos = jnp.arange(kv_len - q_len, kv_len)[:, None]  # [q_len, 1]
        kv_pos = jnp.arange(kv_len)[None, :]  # [1, kv_len]
    ...
    def __call__(
        ...
        # KV caching: keep cache in num_kv_heads space (matches HF DynamicCache behavior).
        if cache is not None:
            k_cache, v_cache = cache  # both [B, num_kv_heads, cache_len, head_dim]
        ...
        # Repeat K/V heads for GQA only for attention compute.
        key_states_for_attn = self._repeat_kv(key_states, self.num_key_value_groups)
        value_states_for_attn = self._repeat_kv(value_states, self.num_key_value_groups)

        # Compute attention weights with scaling
        attn_weights = jnp.matmul(query_states, key_states_for_attn.transpose(0, 1, 3, 2)) * self.scaling
        ...
        # Fully-masked query rows can produce NaNs; zero them out to match
        # expected "no-attention" behavior during padded fixed-length decoding.
        attn_weights = jnp.nan_to_num(attn_weights, nan=0.0, posinf=0.0, neginf=0.0)

        # Apply dropout in a trace-safe way; Dropout handles deterministic mode internally.
        ...
        attn_output = jnp.matmul(attn_weights, value_states_for_attn)

        # --- Reshape and Output --- #
        # Transpose back to [B, q_len, num_heads, head_dim]
        attn_output = attn_output.transpose(0, 2, 1, 3)
        # Reshape to [B, q_len, num_heads * head_dim] before output projection.
        # Gemma variants can have hidden_size != num_heads * head_dim.
...
class Gemma3DecoderLayer(nnx.Module):
    ...
    def __init__(self, layer_idx: int, config: Gemma3Config, *, rngs: nnx.Rngs) -> None:
        super().__init__()
        # Match configured per-layer attention schedule (HF uses sliding with periodic full layers).
        layer_type = config.layer_types[layer_idx]
        attention_type = "global" if layer_type == "full_attention" else "local"
...
class Gemma3ForCausalLM(GenerationMixin, BaseModel):
    ...
    def __call__(
        ...
        # Match HF Gemma3TextScaledWordEmbedding: scale embeddings by sqrt(hidden_size)
        # in model dtype precision (BF16 rounding is part of numerics).
        embed_scale = jnp.asarray(float(self.config.hidden_size) ** 0.5, dtype=hidden_states.dtype)
        ...
        # Pass 2D padding mask to attention layers so each layer can correctly combine:
        # 1) causal mask, 2) optional sliding-window mask, and 3) padding mask.
        attn_padding_mask = padding_mask_2d
        ...
        for i, layer in enumerate(self.layers):
            ...
            hidden_states, updated_layer_cache = layer(
                hidden_states,
                position_ids,
                attention_mask=attn_padding_mask,
                cache=layer_cache,
        ...
        # GenerationMixin expects logits tensor from self(...).
        # Return tuple only when cache behavior is explicitly requested.
    ...
    def convert_weights_from_hf(
        ...
        """Convert HuggingFace Gemma3 weights into this model state.

        Expected HF key patterns:
        ...
        seen_layer: dict[int, set[str]] = {}

        for wholekey, tensor in weights:
            keys = wholekey.split(".")
            if len(keys) < 3 or keys[0] != "model":
                continue

            if keys[1] == "embed_tokens" and keys[2] == "weight":
                state["embed_tokens"]["embedding"].value = tensor  # type: ignore[index]
                needed["embed_tokens"] = True
                continue

            if keys[1] == "norm" and keys[2] == "weight":
                state["norm"]["weight"].value = tensor  # type: ignore[index]
                needed["norm"] = True
                continue

            if keys[1] != "layers" or len(keys) < 5:
                continue

            layer_idx = int(keys[2])
            seen_layer.setdefault(layer_idx, set())

            # model.layers.{i}.self_attn.*
            if keys[3] == "self_attn":
                if keys[4] in {"q_proj", "k_proj", "v_proj", "o_proj"} and len(keys) >= 6 and keys[5] == "weight":
                    state["layers"][layer_idx]["self_attn"][keys[4]]["kernel"].value = tensor.T  # type: ignore[index]
                    seen_layer[layer_idx].add(f"self_attn.{keys[4]}")
                    continue
                if keys[4] in {"q_norm", "k_norm"} and len(keys) >= 6 and keys[5] == "weight":
                    state["layers"][layer_idx]["self_attn"][keys[4]]["weight"].value = tensor  # type: ignore[index]
                    seen_layer[layer_idx].add(f"self_attn.{keys[4]}")
                    continue

            # model.layers.{i}.mlp.*
            if keys[3] == "mlp" and len(keys) >= 6 and keys[5] == "weight":
                if keys[4] in {"gate_proj", "up_proj", "down_proj"}:
                    state["layers"][layer_idx]["mlp"][keys[4]]["kernel"].value = tensor.T  # type: ignore[index]
                    seen_layer[layer_idx].add(f"mlp.{keys[4]}")
                    continue

            # model.layers.{i}.*layernorm.weight
            if len(keys) >= 5 and keys[4] == "weight":
                if keys[3] in {
                    "input_layernorm",
                    ...
                    state["layers"][layer_idx][keys[3]]["weight"].value = tensor  # type: ignore[index]
                    seen_layer[layer_idx].add(keys[3])
                    continue

        if not all(needed.values()):
            missing = [k for k, ok in needed.items() if not ok]
            raise RuntimeError(f"Gemma3 HF conversion missing required top-level weights: {missing}")
        ...
        max_seen_layer = max(seen_layer.keys()) if seen_layer else -1
        for i in range(self.config.num_hidden_layers):
            got = seen_layer.get(i, set())
            missing = expected_per_layer - got
            if missing:
                # If the instantiated config has more layers than the checkpoint, neutralize extras
                # so forward pass remains stable instead of using random params.
                if i > max_seen_layer:
                    zeros = jnp.zeros
                    state["layers"][i]["self_attn"]["q_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["q_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["q_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["self_attn"]["k_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["k_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["k_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["self_attn"]["v_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["v_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["v_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["self_attn"]["o_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["o_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["o_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["self_attn"]["q_norm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["q_norm"]["weight"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["q_norm"]["weight"].value.dtype,
                    )
                    state["layers"][i]["self_attn"]["k_norm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["self_attn"]["k_norm"]["weight"].value.shape,
                        dtype=state["layers"][i]["self_attn"]["k_norm"]["weight"].value.dtype,
                    )
                    state["layers"][i]["mlp"]["gate_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["mlp"]["gate_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["mlp"]["gate_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["mlp"]["up_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["mlp"]["up_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["mlp"]["up_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["mlp"]["down_proj"]["kernel"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["mlp"]["down_proj"]["kernel"].value.shape,
                        dtype=state["layers"][i]["mlp"]["down_proj"]["kernel"].value.dtype,
                    )
                    state["layers"][i]["input_layernorm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["input_layernorm"]["weight"].value.shape,
                        dtype=state["layers"][i]["input_layernorm"]["weight"].value.dtype,
                    )
                    state["layers"][i]["post_attention_layernorm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["post_attention_layernorm"]["weight"].value.shape,
                        dtype=state["layers"][i]["post_attention_layernorm"]["weight"].value.dtype,
                    )
                    state["layers"][i]["pre_feedforward_layernorm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["pre_feedforward_layernorm"]["weight"].value.shape,
                        dtype=state["layers"][i]["pre_feedforward_layernorm"]["weight"].value.dtype,
                    )
                    state["layers"][i]["post_feedforward_layernorm"]["weight"].value = zeros(  # type: ignore[index]
                        state["layers"][i]["post_feedforward_layernorm"]["weight"].value.shape,
                        dtype=state["layers"][i]["post_feedforward_layernorm"]["weight"].value.dtype,
                    )
                    continue
                raise RuntimeError(f"Gemma3 HF conversion missing layer {i} weights: {sorted(missing)}")
