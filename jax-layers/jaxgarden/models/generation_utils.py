...
class GenerationMixin(_Base):
    ...
    def generate(
        ...
        if not use_jit:
            # Eager generation path (non-jitted): use a straightforward autoregressive
            # loop over the current prefix. This avoids fixed-length scan artifacts and
            # mirrors standard greedy/sampling semantics used by HF generation.
            current_ids = input_ids
            finished = finished_sequences

            steps = max_length - initial_seq_len
            for _ in range(steps):
                current_len = int(current_ids.shape[1])
                attn = jnp.ones((batch_size, current_len), dtype=jnp.bool_)
                ...
                next_token = sample_logits(
                    ...
                    min_p=min_p,
                    do_sample=do_sample,
                ).astype(current_ids.dtype)

                if _eos_token_id is not None:
                    newly_finished = (next_token == _eos_token_id) & (~finished)
                    finished = finished | newly_finished
                    output_token = jnp.where(finished, _pad_token_id, next_token)
                else:
                    output_token = next_token

                current_ids = jnp.concatenate([current_ids, output_token[:, None]], axis=1)

                if bool(jnp.all(finished)):
                    break

            # Ensure output has exactly max_length columns.
            if current_ids.shape[1] < max_length:
                pad_cols = max_length - current_ids.shape[1]
                pads = jnp.full((batch_size, pad_cols), _pad_token_id, dtype=current_ids.dtype)
                current_ids = jnp.concatenate([current_ids, pads], axis=1)
            else:
                current_ids = current_ids[:, :max_length]

            return cast(jnp.ndarray, current_ids)
