...
DEFAULT_MODEL_ID = "trl-internal-testing/small-Qwen2ForCausalLM-2.5"

def npy(x, dtype=np.float16):
    if isinstance(x, torch.Tensor):
        # Some Qwen2 test weights/logits are bf16. NumPy cannot always ingest
        # torch.bfloat16 directly, so cast in torch first.
...
def apply_qwen2_rope(q, k, position_ids, rope_theta):
    """
    q: [B,H,T,DH]
    k: [B,KVH,T,DH]
    position_ids: [B,T]

    Qwen2/Llama-style RoPE:
...
def make_qwen2_forward(cfg):
    ...
    def small_qwen2_forward(input_ids, position_ids, *params):
        """
        params:
        ...
        for _ in range(n_layer):
            ...
            q, k = apply_qwen2_rope(q, k, position_ids, rope_theta)
...
def extract_params(model):
    ...
    embed_w = npy(sd["model.embed_tokens.weight"])

    if "lm_head.weight" in sd:
        lm_head_w = npy(sd["lm_head.weight"])
    else:
        lm_head_w = embed_w.copy()
...
def bench_ms(name, fn, warmup=10, iters=50, block=None):
...
def compare(name, ref, got):
    ...
    # Small Qwen2 is bigger and may accumulate more FP16 vs PyTorch/FP32 drift.
    ok = (
        np.isfinite(ref).all()
...
def make_inputs(tok, cfg, prompt, max_length):
    try:
        ...
        raw = [1, 150, 160, 170, 180, 190, 200, 210]
...
def main():
    ...
    ap.add_argument("--save-prefix", default="small_qwen2")
    ...
    with torch.no_grad():
        ...
        torch_logits = torch_logits_t.detach().float().cpu().numpy().astype(np.float32)
    ...
    for i, p in enumerate(params_np[:16]):
    ...
    print("\nRunning manual JAX small Qwen2...")
    ...
    if ok_cactus_jax:
        print("SMALL-QWEN2 CACTUS TEST PASSED ✅")
        print("This proves a larger Qwen2/RoPE/RMSNorm model can run through JAX -> StableHLO -> Cactus.")
    else:
        print("SMALL-QWEN2 CACTUS TEST FAILED ❌")
